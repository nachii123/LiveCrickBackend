package com.cricket_Api.cricketScore.services.impl;

import com.cricket_Api.cricketScore.Entity.Matches;
import com.cricket_Api.cricketScore.Repository.MatchRepo;
...
//                update the match in database
    ...
    private void updateMatchInDb(Matches match1) {
        Matches matches = this.matchRepo.findByTeamHeading(match1.getTeamHeading()).orElse(null);
        if(matches==null){
            matchRepo.save(match1);
        }else{
            match1.setMatchId(matches.getMatchId());
            matchRepo.save(match1);
